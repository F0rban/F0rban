...
- **Header** : barre fine, logo-mot Fraunces + coordonnées de Molines en mono-data (« 44°58′N 6°51′E — 1 813 m »), nav mono-label, lien actif souligné `--filet-peint` ocre. Sticky, fond chaux à 92 % + `backdrop-filter: none` (perf) — le grain fixe passe dessus. CTA persistant à droite. Mobile : `<dialog>` plein écran chaux, nav en display.
